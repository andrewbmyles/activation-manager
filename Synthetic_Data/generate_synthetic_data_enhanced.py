#!/usr/bin/env python3
"""
Enhanced Canadian Consumer Behavior Synthetic Data Generator

This script generates synthetic consumer behavior data using actual metadata files
and realistic PRIZM segmentation profiles.
"""
...
class EnhancedSyntheticDataGenerator:
    def __init__(self, num_records: int = 100000, metadata_dir: str = "Variable Metadata"):
        ...
        # Validate PRIZM percentages
        self._validate_prizm_percentages()
        
    def _validate_prizm_percentages(self):
        """Ensure PRIZM segment percentages sum to approximately 100%."""
        total_pct = sum(seg["percentage"] for seg in self.prizm_segments.values())
        
        if abs(total_pct - 100) > 0.1:
            logger.warning(f"PRIZM percentages sum to {total_pct:.2f}%, normalizing...")
            # Normalize to 100%
            for seg in self.prizm_segments.values():
                seg["percentage"] = seg["percentage"] / total_pct * 100
    
    def generate_postal_code(self) -> str:
        """Generate a valid Canadian postal code based on population distribution."""
        ...
        # Valid letters for postal codes (excluding D, F, I, O, Q, U)
    ...
    def generate_coordinates(self, postal_code: str) -> Tuple[float, float]:
        ...
        if prefix in self.postal_code_distribution:
            ...
            if urban_type == "urban":
                # Smaller variation for urban areas (roughly 20km radius)
                lat_var = np.random.normal(0, 0.1)
                lon_var = np.random.normal(0, 0.1)
            elif urban_type == "suburban":
                # Medium variation for suburban areas (roughly 40km radius)
                lat_var = np.random.normal(0, 0.2)
                lon_var = np.random.normal(0, 0.2)
            else:  # rural
                # Larger variation for rural areas (roughly 100km radius)
            ...
            # Fallback for any undefined prefixes
    ...
    def generate_index_value(self, segment_code: str, variable_name: str) -> int:
        ...
        Uses sophisticated modeling to create realistic distributions.
        """
    ...
    def generate_base_population(self, segment_code: str) -> int:
        """Generate base population values (for NBAS12HP, SVBAS15HP type variables)."""
        segment_profile = self.prizm_segments[segment_code]["profile"]
        
        # Base on income level and urban density
        income_factor = segment_profile["income_level"] / 3.0  # Normalize to ~1.0
    ...
    def add_spatial_correlation(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df.columns:
            if col not in ['PostalCode', 'LATITUDE', 'LONGITUDE', 'PRIZM_SEGMENT', 'FSA']:
        ...
        for col in numeric_cols:
            ...
            fsa_stats = df.groupby('FSA')[col].agg(['mean', 'std']).fillna(0)
            
            # Apply smoothing for each FSA
            for fsa in df['FSA'].unique():
                fsa_mask = df['FSA'] == fsa
                fsa_mean = fsa_stats.loc[fsa, 'mean']
                
                if fsa_mask.sum() > 1:  # Only smooth if multiple records in FSA
                    original_values = df.loc[fsa_mask, col].values
                    smoothed_values = (
                        correlation_strength * fsa_mean + 
                        (1 - correlation_strength) * original_values
                    )
                    df.loc[fsa_mask, col] = smoothed_values.round().astype(int)
    ...
    def validate_data(self, df: pd.DataFrame) -> Dict[str, any]:
        ...
        validation_results = {
            ...
            "postal_codes": {
                ...
                "duplicates": len(df) - df['PostalCode'].nunique()
            },
            "indices": {
                "overall_mean": 0,
        ...
        for col in df.columns:
            if col not in ['PostalCode', 'LATITUDE', 'LONGITUDE', 'PRIZM_SEGMENT']:
                if not col.endswith('HP'):  # Skip base population variables
                    index_cols.append(col)
        
        if index_cols:
            all_indices = df[index_cols].values.flatten()
            ...
            validation_results["indices"]["variables_checked"] = len(index_cols)
            
            # Check if mean is close to 100
            if abs(overall_mean - 100) > 5:
                validation_results["quality_checks"]["all_passed"] = False
        ...
        for segment, actual_pct in actual_dist.items():
            ...
            if difference > 0.5:  # More than 0.5% difference
                validation_results["quality_checks"]["all_passed"] = False
                validation_results["quality_checks"]["failed_checks"].append(
    ...
    def generate_dataset(self) -> pd.DataFrame:
        ...
        all_variables = self.metadata_loader.get_all_variable_names()
        logger.info(f"Total variables to generate: {len(all_variables)}")
        ...
        for i in range(self.num_records):
            ...
            for var in all_variables:
                if var.upper() in ['CODE', 'GEO', 'FSALDU']:
                    # Skip geographic identifiers
                    continue
                elif var.endswith('HP') or 'POPULATION' in var.upper():
    ...
    def save_dataset(self, df: pd.DataFrame, output_dir: str = "output"):
        ...
        metadata_summary = {
            ...
            "variables": {
                ...
                "by_category": self.metadata_loader.get_variable_count_by_category()
            },
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate synthetic Canadian consumer behavior data"
    )
    parser.add_argument(
        '-n', '--num-records',
        type=int,
    ...
    validation = generator.validate_data(df)
    print(f"\nValidation Summary:")
