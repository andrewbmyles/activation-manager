...
FIXED: Proper static file serving for React apps
"""
...
def fix_asset_paths(html_content, base_path='/activation-manager'):
    """Fix relative asset paths in HTML to use absolute paths"""
    import re
    
    # Fix href paths
    html_content = re.sub(r'href="\./', f'href="{base_path}/', html_content)
    # Fix src paths
    html_content = re.sub(r'src="\./', f'src="{base_path}/', html_content)
    ...
    try:
        ...
        if action == 'process':
            ...
            for var in all_variables:
                category = var.get('category', 'Demographics').lower()
                category_key = 'demographic'  # default
                
                if 'behavior' in category:
                    category_key = 'behavioral'
                elif 'psycho' in category:
                    category_key = 'psychographic'
                elif 'financial' in category or 'income' in category:
                ...
                suggested_variables[category_key].append({
                    ...
                    'category': var.get('category', 'Demographics'),
...
def serve_activation_manager():
    """Serve Activation Manager app with fixed paths"""
    build_path = os.path.join(os.path.dirname(__file__), 'audience-manager', 'build')
    ...
    if os.path.exists(index_path):
        ...
        # Fix the asset paths
        html_content = fix_asset_paths(html_content, '/activation-manager')
        
        return Response(html_content, mimetype='text/html')
    
    return "Activation Manager not found", 404
...
def serve_activation_manager_assets(path):
    ...
    # Handle other assets
    if path in ['favicon.ico', 'manifest.json', 'robots.txt', 'logo192.png', 'logo512.png', 'headshot.jpg']:
        file_path = os.path.join(build_path, path)
        if os.path.exists(file_path):
            return send_from_directory(build_path, path)
    
    # For all other paths, serve the index.html with fixed paths (for React Router)
    index_path = os.path.join(build_path, 'index.html')
    if os.path.exists(index_path):
        with open(index_path, 'r') as f:
            html_content = f.read()
...
def serve_tobermory_web():
    """Serve Tobermory Web app root"""
    build_path = os.path.join(os.path.dirname(__file__), 'tobermory-web', 'build')
    index_path = os.path.join(build_path, 'index.html')
    ...
    if os.path.exists(index_path):
        ...
        # Fix the asset paths for root
        html_content = html_content.replace('href="./', 'href="/')
        html_content = html_content.replace('src="./', 'src="/')
        
        return Response(html_content, mimetype='text/html')
    
    return "Tobermory Web not found", 404

@app.route('/<path:path>')
def serve_tobermory_web_assets(path):
    """Serve Tobermory Web static assets"""
    build_path = os.path.join(os.path.dirname(__file__), 'tobermory-web', 'build')
    ...
    # For all other paths, serve the index.html (for React Router)
    index_path = os.path.join(build_path, 'index.html')
    if os.path.exists(index_path):
        with open(index_path, 'r') as f:
            html_content = f.read()
